...
class Classifier:
    def __init__(self, model_path, tags_path):
        ...
        rd.close()
        tags = tags.split('\n')[0:-1]
        self.tags_length = len(tags)
        self.tags = np.array(tags)

    def get_tags(self, img):
        data = toml.load('config.toml')
        threshold=data['deep_booru_threshold']
        image = img / 255
        image_shape = image.shape
        image = image.reshape((1, image_shape[0], image_shape[1], image_shape[2]))
        y = self.model.predict(image, verbose=0)[0]
        # 6891
        y = y[0:self.tags_length]
        passed = y > threshold
        found_tags = self.tags[passed]
        confidences = y[passed]
        sorted = np.argsort(confidences * -1)
        found_tags = found_tags[sorted]
        confidences = confidences[sorted]
        deep_booru_limit = data['deep_booru_limit']
        found_tags = found_tags[0:min(len(found_tags), deep_booru_limit)]
        confidences = confidences[0:min(len(confidences), deep_booru_limit)]
        return found_tags, confidences
...
class TagsFrame:
    def __init__(self, master, callback_func=None, indicator=False, description='BLANK', col=0,
                 highlightbackground=None, highlightthickness=2):
        self.indicator = indicator
        self.tag_btn_list = []
        self.tag_dict = {}
        self.top_row_index = 1
        self.master = master
        self.frame = Frame(master=master, highlightbackground=highlightbackground,
...
class Pic:
    ...
    def get_deep_booru_checked(self):
        return self.deep_booru_checked
    def set_deep_booru_checked(self,state):
        self.deep_booru_checked=state
...
class PicFrame:
    ...
    def reload(self):
        self.load(reload_inference=True)
    ...
    def load(self,reload_inference=False):
        ...
        get_deep_booru_checked=current_pic.get_deep_booru_checked()
        if (not current_pic.get_deep_booru_checked()) or reload_inference:
            current_pic.set_deep_booru_checked(True)
            print("running deeppboru inference on ",image_path)
            img_rgb=img[:, :, ::-1]
            tags,conf=self.deepbooru.get_tags(img_rgb)
            for tag_ in tags:
                if not (tag_ in tag_list):
                    self.current_tag_frame.add_btn((tag_, True))
...
def enable_thresh(event):
    global threshold_entry
    threshold_entry.configure(state='normal')


def enter_thresh(event):
    global threshold_entry, threshold
    val = threshold_entry.get()
    try:
        val = float(val)
    except:
        print("not a number")
    ...
    val = float(val)
    if val > 1:
        print("choose number between 0 and 1")
        return
...
# threshold_lbl = Label(master=controlframe, text='deepbooru threshold (press enter):')
# threshold_lbl.grid(column=3, row=0)
# threshold_entry = Entry(master=controlframe, width=10)
# threshold_entry.grid(column=4, row=0)
# threshold_entry.configure(state='disabled')
# threshold_entry.insert(END, '.1')
# reload_btn = Button(master=controlframe, text='reload (re does deepbooru inference)',command=pic_subframe.reload)
# reload_btn.grid(column=0, row=1, padx=5, pady=5)
# threshold_entry.bind('<ButtonPress-1>', enable_thresh)
# threshold_entry.bind('<Return>', enter_thresh)
